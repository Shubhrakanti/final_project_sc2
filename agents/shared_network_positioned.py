...
class DQN(base_agent.BaseAgent):
    ...
    def __init__(self,
        ...
        self.network = nets.PlayerRelativeMovementCNNPositioned(
        ...
        if self.training:
            self.target_net = nets.PlayerRelativeMovementCNNPositioned(
                spatial_dimensions=feature_screen_size,
                learning_rate=self.learning_rate,
                name="DQNTarget")
    ...
    def step(self, obs):
        ...
        other_marines = [marine for marine in marines if marine.x != marine_unit.x and marine.y != marine_unit.y]
        other_marine_xy = other_marines[0] if len(other_marines) > 0 else marine_unit
        other_marine_xy = (other_marine_xy.x, other_marine_xy.y)
        ...
        if FUNCTIONS.Move_screen.id in obs.observation.available_actions:
            ...
            if self.training:
                ...
                x, y, action = self._epsilon_greedy_action_selection(state, other_marine_xy)
    ...
    def _epsilon_greedy_action_selection(self, state, other_marines, epsilon=None):
        ...
        if epsilon > np.random.rand():
            ...
            other_marines = np.expand_dims(other_marines, 0)

            q_values = self.sess.run(
                self.network.flatten,
                feed_dict={self.network.inputs: inputs,
                            self.network.other_marines: other_marines})
    ...
    def _train_network(self):
        states, actions, targets, other_marines = self._get_batch()
        self.network.optimizer_op(self.sess, states, actions, targets, other_marines)
    ...
    def _get_batch(self):
        ...
        eye = np.eye(np.prod(feature_screen_size))
        other_marines = np.array([eye[np.ravel_multi_index(each[4], feature_screen_size)] for each in batch])

        # one-hot encode actions
        actions = np.eye(np.prod(feature_screen_size))[actions]
 
        other_marines = np.reshape(other_marines, (-1, *feature_screen_size))

        # get targets
        next_outputs = self.sess.run(
            self.target_net.output,
            feed_dict={self.target_net.inputs: next_states,
                        self.target_net.other_marines: other_marines})

        targets = [rewards[i] + self.discount_factor * np.max(next_outputs[i])
                   for i in range(self.batch_size)]

        return states, actions, targets, other_marines
