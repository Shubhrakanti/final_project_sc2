...
class PlayerRelativeMovementCNNSeparateOutputs(object):
    """Uses feature_screen.player_relative to assign q value to movements."""
    ...
    def optimizer_op(self, sess, states, actions, targets, marine):
        """Perform one iteration of gradient updates."""
        loss, _ = sess.run(
            [self.loss, self.optimizer],
            feed_dict={self.inputs: states,
    ...
    def _build(self):
        ...
        with tf.variable_scope(self.name):
            ...
            self.t1 = tf.reshape(self.out, [-1, *self.spatial_dimensions])
            self.t2 = tf.layers.flatten(self.t1, name="flat")
            self.output = tf.reshape(self.t2, [-1, 2, 7056])

            # value estimate trackers for summaries
            self.max_q = tf.reduce_max(self.output, name="max")
            self.mean_q = tf.reduce_mean(self.output, name="mean")

            indices = tf.stack([tf.range(tf.size(self.marines)), self.marines], axis=-1)

            transposed = tf.transpose(self.out, perm=[0, 3, 1, 2], name="t2")
            gathered = tf.gather_nd(transposed, tf.cast(indices, tf.int32))
            self.result = tf.expand_dims(gathered, -1, name="res")

            # optimization: MSE between state predicted Q and target Q
            self.prediction = tf.reduce_sum(
                tf.multiply(tf.layers.flatten(self.result), self.actions),
                axis=1,
                name="prediction")
...
class PlayerRelativeMovementCNNPositioned(object):
    def __init__(self,
                 spatial_dimensions,
                 learning_rate,
                 save_path=None,
    ...
    def optimizer_op(self, sess, states, actions, targets, other_marines):
        """Perform one iteration of gradient updates."""
        loss, _ = sess.run(
            [self.loss, self.optimizer],
            feed_dict={self.inputs: states,
                       self.actions: actions,
                       self.targets: targets,
                       self.other_marines: other_marines})
    ...
    def _build(self):
        ...
        with tf.variable_scope(self.name):
            ...
            self.other_marines = tf.placeholder(
                tf.int32,
                [None, *self.spatial_dimensions],
                name="other_marines"
            )
            ...
            self.other_marines_oh = tf.one_hot(
                self.other_marines,
                depth=1,
                axis=-1,
                name="one_hot_other")

            self.other_marine_features = tf.layers.conv2d(
                inputs = self.other_marines_oh,
                filters=1,
                kernel_size = [1,1],
                strides=[1, 1],
                padding="SAME",
                name="other_marine_feats"
            )

            self.concat = tf.concat((self.embed, self.other_marine_features), 3, name="concat")
