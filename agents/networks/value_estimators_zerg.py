...
class FullyConvNetZerg(object):
    ...
    def _build(self):
        ...
        with tf.variable_scope(self.name):
            ...
            # spatial output layer
            self.spatial_output_move = tf.layers.conv2d(
                inputs=self.conv2_activation,
                filters=1,
                kernel_size=[1, 1],
                strides=[1, 1],
            ...
            # value estimate trackers for summaries
            self.max_q = tf.reduce_max(self.spatial_flatten_move+self.spatial_flatten_attack, name="max")
            self.mean_q = tf.reduce_mean(self.spatial_flatten_move+self.spatial_flatten_attack, name="mean")

            # optimization: MSE between state predicted Q and target Q
            self.prediction = tf.reduce_sum(
                tf.multiply(self.spatial_flatten_attack+self.spatial_flatten_move, self.actions),
                axis=1,
                name="prediction")
