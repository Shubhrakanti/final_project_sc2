...
class AtariNetSimplified(object):
...
class AtariControlNet(object):
    ...
    def _build(self):
        ...
        units = self.screen_dimensions
        self.x1 = tf.layers.dense(
                    inputs=self.state_representation,
                    units=units[0],
                    activation=tf.nn.softmax)

        self.y1 = tf.layers.dense(
                    inputs=self.state_representation,
                    units=units[1],
                    activation=tf.nn.softmax)
        ...
        self.y2 = tf.layers.dense(
                    inputs=self.state_representation,
                    units=units[1],
                    activation=tf.nn.softmax)


        self.arg_placeholder_x1 = tf.placeholder(
                    tf.float32,
                    shape=[None, units[0]],
                    name="x1")

        self.arg_placeholder_y1 = tf.placeholder(
                    tf.float32,
                    shape=[None, units[1]],
                    name="y1")

        self.arg_placeholder_x2 = tf.placeholder(
                    tf.float32,
                    shape=[None, units[0]],
                    name="x2")

        self.arg_placeholder_y2 = tf.placeholder(
                    tf.float32,
                    shape=[None, units[1]],
                    name="y2")
    ...
    def _build_optimization(self):
        ...
        arg_prob = tf.reduce_sum(
                self.arg_placeholder_x1 * self.x1)
        nonzero_probs = tf.cond(
                tf.logical_not(tf.equal(arg_prob, 0)),
                true_fn=lambda: arg_prob,
                false_fn=lambda: 1.)
        ...
        arg_prob = tf.reduce_sum(
                self.arg_placeholder_y1 * self.y1)
        nonzero_probs = tf.cond(
                tf.logical_not(tf.equal(arg_prob, 0)),
                true_fn=lambda: arg_prob,
                false_fn=lambda: 1.)
        ...
        arg_prob = tf.reduce_sum(
                self.arg_placeholder_x2 * self.x2)
        nonzero_probs = tf.cond(
                tf.logical_not(tf.equal(arg_prob, 0)),
                true_fn=lambda: arg_prob,
                false_fn=lambda: 1.)
        ...
        arg_prob = tf.reduce_sum(
                self.arg_placeholder_y2 * self.y2)
        nonzero_probs = tf.cond(
                tf.logical_not(tf.equal(arg_prob, 0)),
                true_fn=lambda: arg_prob,
                false_fn=lambda: 1.)
