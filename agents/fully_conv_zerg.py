# -*- coding: utf-8 -*-
"""
Created on Wed May  1 13:40:47 2019
...
# local submodule
import agents.networks.value_estimators_zerg as nets
...
class FullyConvAgent(base_agent.BaseAgent):
    ...
    def __init__(self,
        ...
        self.network = nets.FullyConvNetZerg(
    ...
    def step(self, obs):
        ...
        if FUNCTIONS.Move_screen.id in obs.observation.available_actions and FUNCTIONS.Attack_screen.id in obs.observation.available_actions:
            ...
            if self.training:
                ...
                x,y, action, eps = self._epsilon_greedy_action_selection(screen)
            ...
            if self.indicate_nonrandom_action and eps == "nonrandom":
    ...
    def _epsilon_greedy_action_selection(self, state, epsilon=None):
        ...
        if epsilon > np.random.rand():
            ...
            return x, y, "None", "random"

        else:
            inputs = np.expand_dims(state, 0)

            q_values_move = self.sess.run(
                    self.network.spatial_flatten_move,
                    feed_dict={self.network.inputs: inputs})
            move_max_index = np.argmax(q_values_move)

            
            q_values_attack = self.sess.run(
                    self.network.spatial_flatten_move,
                    feed_dict={self.network.inputs: inputs})
